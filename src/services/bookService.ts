...
import { Book, GoogleBook, NewBook } from 'lib/models/Book.interface';
...
function query(filterBy = getEmptyBookFilter()) {
  let filteredBooks = gBooks;

  if (filterBy.publishedDate.value) {
    const {
      publishedDate: { value, before },
    } = filterBy;
    filteredBooks = filteredBooks.filter((currBook: Book) => {
      const bookMs = new Date(currBook.publishedDate).getTime();
      const filterMs = new Date(value).getTime();
      return before ? bookMs < filterMs : bookMs > filterMs;
    });
  }

  if (filterBy.amount.value) {
    const {
      amount: { value, lessThan },
    } = filterBy;
    filteredBooks = filteredBooks.filter((currBook: Book) => {
      return lessThan
        ? currBook.listPrice.amount < value
        : currBook.listPrice.amount > value;
    });
  }

  if (filterBy.freeTxt) {
    console.log('filteredBooksBefore: ', [...filteredBooks]);
    filteredBooks = filteredBooks.filter((currBook: Book) => {
      type SearchField = 'title' | 'subtitle' | 'description';
      const searchFields: SearchField[] = ['title', 'subtitle', 'description'];
      return searchFields.some((searchField: SearchField) => {
        return currBook[searchField].includes(filterBy.freeTxt);
      });
    });
    console.log('filtered books after: ', filteredBooks);
  }

  return Promise.resolve(filteredBooks);
}

function getById(bookId: string) {
  const book = gBooks.find((currBook) => currBook.id === bookId);
  return Promise.resolve(book);
}

function deleteBook(bookId: string) {
  gBooks = gBooks.filter((currBook) => currBook.id !== bookId);
  return Promise.resolve();
}

function save(book: Book): Promise<Book> {
  //edit
  gBooks = gBooks.map((currBook) =>
    currBook.id === book.id ? book : currBook
  );
  return Promise.resolve(book);
}

function addGoogleBook(book: GoogleBook) {
  ...
  gBooks = [newBook, ...gBooks];
  storageService.saveToStorage('books', gBooks);
  return Promise.resolve(newBook);
}

function _loadBooks() {
  gBooks = storageService.loadFromStorage('books');
  if (!gBooks) createBooks();
    ...
    gBooks = [
      {
        ...
        publishedDate:
          'Tue Jul 23 2020 02:00:01 GMT+0200 (Israel Standard Time)',
        description:
      ...
      },
      {
        ...
        authors: ['Barbara Cartland'],
        publishedDate:
          'Tue Jan 29 1992 02:00:01 GMT+0200 (Israel Standard Time)',
        description:
      ...
      },
      {
        ...
        authors: ['Dr. Seuss'],
        publishedDate:
          'Tue Jan 23 1990 02:00:01 GMT+0200 (Israel Standard Time)',
        description:
...
async function getGoogleBooks(bookName: string) {
...
function getEmptyBookFilter(): BookFilter {
  return {
    publishedDate: {
      value: '',
      before: false,
